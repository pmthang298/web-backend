...
func (s *Server) ForgotPassword(c *gin.Context) {
	...
	resetDetails, err := resetPassword.SaveDatails(s.DB)
	if err != nil {
...
func (s *Server) ResetPassword(c *gin.Context) {
	...
	err = s.DB.Debug().Model(models.ResetPassword{}).Where("token = ?", requestBody["token"]).Take(&resetPassword).Error
	if err != nil {
	...
	if requestBody["new_password"] != "" && requestBody["retype_password"] != "" {
		...
		_, err = resetPassword.DeleteDatails(s.DB)
		if err != nil {
